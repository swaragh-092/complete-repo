...
module.exports = function initModels(sequelize) {
    ...
    // mail
    db.MailQueue = require("./util/mailQueue.model")(
    sequelize,
    Sequelize.DataTypes,
    TABLE_PREFIX,
    commonFields
    ...
    const REQUIRE_HISTORY_TABLE = Object.keys(db)
    .filter(
        (modelName) =>
        db[modelName] && typeof db[modelName].getTableName === "function"
    )
    .map((modelName) => db[modelName].getTableName());
