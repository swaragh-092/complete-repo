...
// ============================================================================
// GET /auth/clients/:clientKey/credentials
// Returns all env-relevant credentials for an approved client.
// Protected by developer email verification (matches original registration).
// Used by: sso-client pull-env (for already-developed / production apps)
// ============================================================================
router.get('/clients/:clientKey/credentials', asyncHandler(async (req, res) => {
  try {
    const { clientKey } = req.params;
    const { developerEmail } = req.query;

    if (!developerEmail) {
      throw new AppError('developerEmail query parameter is required', 400, 'VALIDATION_ERROR');
    }

    // Verify ownership via the original registration request
    const request = await ClientRequest.findOne({
      where: { client_key: clientKey },
      attributes: ['developer_email', 'status']
    });

    if (!request) {
      throw new AppError('No registration request found for this client key', 404, 'NOT_FOUND');
    }

    if (request.developer_email.toLowerCase() !== developerEmail.toLowerCase()) {
      throw new AppError('Developer email does not match the registered email for this client', 403, 'FORBIDDEN');
    }

    if (request.status !== 'approved') {
      throw new AppError(
        `Client is not approved yet. Current status: ${request.status}`,
        400,
        'NOT_APPROVED'
      );
    }
    ...
    const client = await Client.findOne({
      where: { client_key: clientKey },
      include: [{ model: Realm, attributes: ['realm_name', 'display_name'] }]
    });

    if (!client) {
      throw new AppError('Approved client record not found in database', 404, 'NOT_FOUND');
    }

    logger.info(`Credentials retrieved for client: ${clientKey} by ${developerEmail}`);
    ...
    if (error instanceof AppError) throw error;
    logger.error('Failed to retrieve client credentials:', error);
    throw new AppError('Failed to retrieve client credentials', 500, 'FETCH_FAILED', { originalError: error.message });
  }
}));
