...
const buildRefreshCookieOptions = (req, redirectUri, savedOrigin) => {
  ...
  if (!['none', 'lax', 'strict'].includes(sameSite)) {
    ...
    const isCrossSite =
      requestOrigin && redirectOrigin && !sameOrigin(requestOrigin, redirectOrigin);
...
async function validateRedirectUri(redirectUri, client) {
  ...
  try {
    ...
    const allowedUris = [client.redirect_url, client.callback_url].filter(Boolean); // Remove null/undefined
  ...
  asyncHandler(async (req, res, next) => {
    ...
    try {
      ...
      if (redirectUri && !(await validateRedirectUri(redirectUri, client))) {
        ...
        throw new AppError(
          'The redirect URI does not match the configured client redirect URLs',
          400,
      ...
      if (redirectUri && req.session) {
        ...
        req.session.oauth_original_origin =
          req.headers.origin ||
          (req.headers.referer ? extractOrigin(req.headers.referer) : null) ||
          null;
      }
      ...
      throw new AppError('Internal server error', 500, 'LOGIN_INIT_FAILED', {
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!redirectUri) {
      ...
      throw new AppError(
        'Unable to determine a safe redirect URI. Please check client configuration.',
        400,
        'INVALID_REDIRECT_URI'
      );
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const refreshToken =
        req.body.refreshToken || req.headers['x-refresh-token'] || req.cookies?.[`rt_${clientKey}`];
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to retrieve onboarding status', 500, 'STATUS_CHECK_FAILED', {
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (!client) {
        ...
        throw new AppError(
          `Client with key "${clientKey}" does not exist or has not been approved`,
          404,
          'NOT_FOUND'
        );
      }
      ...
      logger.error('❌ Failed to get client config:', error);
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to fetch client configuration', 500, 'FETCH_FAILED', {
        originalError: error.message,
      });
    }
  })
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to get user profile', 500, 'FETCH_FAILED', {
        originalError: error.message,
      });
    }
  })
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const tokenSource = req.body?.refreshToken
      ? 'body'
      : req.headers['x-refresh-token']
        ? 'header'
        : req.cookies?.[`rt_${clientKey}`]
          ? `cookie:rt_${clientKey}`
          : 'NONE';
    ...
    if (!refreshToken) {
      throw new AppError(
        'Refresh token not found in cookies, body, or headers',
        400,
        'MISSING_TOKEN'
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 || response.status === 401) {
          ...
          const clearOpts = buildRefreshCookieOptions(
            req,
            client.redirect_url,
            req.headers.origin || null
          );
      ...
      if (err instanceof AppError) throw err;
      throw new AppError('Internal server error during token refresh', 500, 'TOKEN_REFRESH_ERROR', {
        originalError: err.message,
      });
    }
  })
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (err instanceof AppError) throw err;
      throw new AppError('Failed to create user', 500, 'CREATION_FAILED', {
        originalError: err.message,
      });
    }
  })
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      throw new AppError('Failed to fetch user data', 500, 'FETCH_FAILED', {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (err instanceof AppError) throw err;
      throw new AppError('Failed to assign role', 500, 'ASSIGNMENT_FAILED', {
        originalError: err.message,
      });
    }
  })
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      if (error instanceof AppError) throw error;
      throw new AppError('Failed to fetch audit logs', 500, 'FETCH_FAILED', {
        originalError: error.message,
      });
    }
  })
