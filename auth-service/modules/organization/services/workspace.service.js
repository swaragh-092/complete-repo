...
class WorkspaceService {
    ...
    try {
      ...
      if (!orgMembership) {
        ...
        if (!isSuperAdmin(userRoles)) {
          throw new AppError(
            'You must be a member of the organization to create a workspace',
            403,
            'ORG_ACCESS_DENIED'
          );
        }
      ...
      if (currentCount >= maxWorkspaces) {
        throw new AppError(
          `Organization has reached the maximum limit of ${maxWorkspaces} workspaces`,
          400,
      ...
      if (existingSlug) {
        throw new AppError(
          `Workspace ID '${slug}' already exists in this organization`,
          409,
          'SLUG_COLLISION'
        );
      }
  ...
  async getMembers(workspaceId) {
    try {
      const members = await WorkspaceMembership.findAll({
      ...
      // Enrich with names from Keycloak
      const enrichedMembers = await Promise.all(
        members.map(async (member) => {
          const memberObj = member.toJSON();
          if (memberObj.UserMetadata?.keycloak_id) {
            try {
              const kcUser = await KeycloakService.getUserById(memberObj.UserMetadata.keycloak_id);
              memberObj.name =
              ...
              logger.warn(
                `Failed to fetch name for keycloak_id ${memberObj.UserMetadata.keycloak_id}:`,
                err.message
              );
              memberObj.name = memberObj.UserMetadata.email || 'Unknown';
            }
          } else {
            memberObj.name = memberObj.UserMetadata?.email || 'Unknown';
          }
  ...
  async removeMember({ requesterId, workspaceId, targetUserId }) {
    ...
    try {
      ...
      if (requesterId === targetUserId) {
        ...
        if (adminCount <= 1) {
          throw new AppError(
            'Cannot leave: You are the last admin. Promote someone else first.',
            400,
            'LAST_ADMIN_GUARD'
          );
        }
  ...
  async updateWorkspace({ requesterId, workspaceId, name, description }) {
    ...
    try {
      ...
      if (!membership) {
        throw new AppError(
          'Only workspace admins can edit workspace details',
          403,
          'ACCESS_DENIED'
        );
      }
  ...
  async updateMemberRole({ requesterId, workspaceId, targetUserId, newRole }) {
    ...
    try {
      ...
      if (targetMembership.role === 'admin' && newRole !== 'admin') {
        ...
        if (adminCount <= 1) {
          throw new AppError(
            'Cannot demote: This is the last admin. Promote someone else first.',
            400,
            'LAST_ADMIN_GUARD'
          );
        }
      ...
      await transaction.commit();
      logger.info(
        `User ${targetUserId} role changed to ${newRole} in workspace ${workspaceId} by ${requesterId}`
      );
  ...
  async sendInvitation({ requesterId, workspaceId, email, role, message }) {
    ...
    try {
      ...
      // 5. Get inviter email
      const inviter = await UserMetadata.findByPk(requesterId, {
        attributes: ['email'],
        transaction,
      });
  ...
  async acceptInvitation({ code, userId }) {
    ...
    try {
      ...
      if (!orgMembership) {
        ...
        throw new AppError(
          'You must be a member of the organization first. Contact an admin.',
          403,
          'NOT_IN_ORG'
        );
      }
