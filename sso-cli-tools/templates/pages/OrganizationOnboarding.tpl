...
export default function OrganizationOnboarding() {
  ...
  const fetchOnboardingStatus = async () => {
    try {
      ...
      if (response.data.pending_invitation) {
        ...
        setActiveTab(['create_org', 'flexible'].includes('{{ONBOARDING_FLOW}}') ? 0 : 0); 
      } else {
  ...
  const showCreateTab = ['create_org', 'flexible'].includes('{{ONBOARDING_FLOW}}');
  const showJoinTab = ['invitation_only', 'flexible'].includes('{{ONBOARDING_FLOW}}');
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            {currentTabContent === 'join' && (
              <JoinOrganizationForm 
                ...
                allowDomainMatching={'{{ONBOARDING_FLOW}}' === 'domain_matching'}
