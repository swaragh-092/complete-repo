...
/**
 * POST /api/workspaces/batch-lookup
 * Batch lookup workspace details by IDs (no membership required)
 * Body: { workspace_ids: ["uuid1", "uuid2"] }
 * Returns: { workspaces: [{ id, name, description, slug, org_id }] }
 */
router.post(
  '/batch-lookup',
  asyncHandler(async (req, res) => {
    ...
    if (!Array.isArray(workspace_ids) || workspace_ids.length === 0) {
      return ResponseHandler.error(res, 'workspace_ids must be a non-empty array', 400);
    }

    const workspaces = await Workspace.findAll({
      where: {
        id: { [require('sequelize').Op.in]: workspace_ids },
      },
      attributes: ['id', 'name', 'description', 'slug', 'org_id', 'created_at'],
    });
