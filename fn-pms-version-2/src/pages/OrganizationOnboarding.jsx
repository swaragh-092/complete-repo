...
// Description: Organisation onboarding wizard guiding a new owner through initial configuration.
// Version: 1.0.0
// Modified:
...
import { Box, Card, CardContent, Typography, Button, TextField, Tabs, Tab, CircularProgress, Container, Alert, Paper, Divider, useTheme, alpha } from "@mui/material";
import { Business as BusinessIcon, GroupAdd as JoinIcon, Mail as InviteIcon } from "@mui/icons-material";
...
export default function OrganizationOnboarding() {
  ...
  }, [clientKey, accessToken, fetchOnboardingStatus]);
  ...
  return (
    <Box
      sx={{
        ...
        background: theme.palette.mode === "dark" ? `linear-gradient(135deg, ${alpha(theme.palette.primary.main, 0.2)} 0%, ${alpha(theme.palette.secondary.main, 0.2)} 100%)` : `linear-gradient(135deg, ${theme.palette.primary.main} 0%, ${theme.palette.secondary.main} 100%)`,
        p: 3,
      }}
    >
      <Container maxWidth="md">
        <Card sx={{ borderRadius: 3, overflow: "visible" }}>
          ...
          <CardContent sx={{ p: 4 }}>
            <Tabs value={activeTab} onChange={(e, v) => setActiveTab(v)} centered sx={{ mb: 4, borderBottom: 1, borderColor: "divider" }}>
            ...
            {currentTabContent === "invitation" && onboardingData?.pending_invitation && <PendingInvitation invitation={onboardingData.pending_invitation} onAccept={handleAcceptInvitation} />}

            {currentTabContent === "create" && <CreateOrganizationForm onSubmit={handleCreateOrganization} organizationModel="multi" />}

            {currentTabContent === "join" && <JoinOrganizationForm onSubmit={handleJoinByCode} allowDomainMatching={"flexible" === "domain_matching"} userEmail={onboardingData?.user?.email} />}
          </CardContent>
        </Card>
      </Container>
    </Box>
  ...
  return (
    <Paper variant="outlined" sx={{ p: 3, textAlign: "center" }}>
      ...
      <Box sx={{ display: "flex", gap: 2, justifyContent: "center", mt: 3 }}>
        <Button variant="contained" onClick={() => onAccept(invitation.id)}>
          Accept Invitation
        </Button>
...
function CreateOrganizationForm({ onSubmit }) {
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 500, mx: "auto" }}>
      ...
      <TextField fullWidth label="Organization Name" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} sx={{ mb: 2, mt: 2 }} />
      <TextField fullWidth label="Description" multiline rows={3} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} sx={{ mb: 2 }} />
      <TextField fullWidth label="Website (Optional)" value={formData.website} onChange={(e) => setFormData({ ...formData, website: e.target.value })} sx={{ mb: 3 }} />
...
function JoinOrganizationForm({ onSubmit, allowDomainMatching, userEmail }) {
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: 500, mx: "auto" }}>
      ...
      <TextField fullWidth label="Invitation Code" required value={inviteCode} onChange={(e) => setInviteCode(e.target.value)} helperText="Ask your organization admin for an invitation code" sx={{ mb: 3, mt: 2 }} />
      <Button type="submit" variant="contained" size="large" fullWidth>
        Join Organization
      </Button>
    </Box>
