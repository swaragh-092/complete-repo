...
export default function Callback() {
    ...
    (async () => {
      try {
        ...
        if (accessToken) {
          ...
          // Also set as HttpOnly-like cookie for backend API calls
          // This allows PMS and other backend services to read the token
          setCookie("access_token", accessToken, 7 * 24 * 60 * 60); // 7 days
          ...
          console.log("✅ Tokens saved (localStorage + cookie)");
        } else {
          ...
          setToken(token);
          setCookie("access_token", token, 7 * 24 * 60 * 60);
        }
      } catch (e) {
  ...
  }, [isAuthenticated, navigate, fetchOrganizations, currentOrganization]);
