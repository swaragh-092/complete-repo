...
// Version: 1.0.0
// Modified: By Gururaj, added queryWithLogAudit and queryMultipleWithAuditLog

class AuditLogService {
  //  only for bulk delete send snapshort
  async queryWithLogAudit({
    action,
    req,
    queryCallBack,
    snapshot = null,
    ...
    const result = await sequelize.transaction(async (t) => {
      ...
      console.log(queryExicuted);

      const userId = req.user_id || req.user?.id;
      const organizationId = req.organization_id || req.user?.organization_id;

      // If nothing was done, just return early
      if (
        !queryExicuted ||
        (Array.isArray(queryExicuted) && queryExicuted.length === 0)
      ) {
        return queryExicuted;
      }

      let modelClass = model ?? null;

      // get model for single or bulk update
      if (
        !modelClass &&
        Array.isArray(queryExicuted) &&
        queryExicuted.length > 0
      ) {
        modelClass = queryExicuted[0]?.constructor;
      } else if (
        !modelClass &&
        queryExicuted &&
        typeof queryExicuted === "object"
      ) {
    ...
    const results = await sequelize.transaction(async (t) => {
      ...
      for (const op of operations) {
        ...
        if (typeof op.queryCallBack !== "function") {
          throw new Error(
            "Each operation must provide a queryCallback function.",
          );
        }
        ...
        if (
          op.auditTargetExtractor &&
          typeof op.auditTargetExtractor === "function"
        ) {
        ...
        if (
          ...
          Array.isArray(op.snapshot) &&
          op.snapshot.length > 0 &&
          !modelClass
        ) {
          modelClass = op.snapshot[0]?.constructor;
        } else if (
          op.snapshot &&
          typeof op.snapshot === "object" &&
          !modelClass
        ) {
