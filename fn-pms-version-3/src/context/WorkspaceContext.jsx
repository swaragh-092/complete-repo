...
// Description: React context and provider for the currently active workspace (department) data.
// Version: 1.0.0
// Modified:
...
import { getWorkspaces, createWorkspace as apiCreateWorkspace, updateWorkspace as apiUpdateWorkspace, deleteWorkspace as apiDeleteWorkspace, getCurrentWorkspace, setCurrentWorkspace as persistWorkspace, clearCurrentWorkspace } from "../api/workspaces";
...
export function WorkspaceProvider({ children }) {
  ...
  const updateWorkspace = useCallback(
    async (workspaceId, updates) => {
      ...
      try {
        ...
        setWorkspaces((prev) => prev.map((ws) => (ws.id === workspaceId ? normalized : ws)));
   ...
   * @returns {boolean}
   */
  const canEdit = useMemo(() => hasPermission(WORKSPACE_ROLES.EDITOR), [hasPermission]);

  /**
   * Check if user is admin
   * @returns {boolean}
   */
  const isAdmin = useMemo(() => hasPermission(WORKSPACE_ROLES.ADMIN), [hasPermission]);
  ...
  const contextValue = useMemo(
    ...
    [workspaces, currentWorkspace, loading, error, fetchWorkspaces, createWorkspace, updateWorkspace, deleteWorkspace, selectWorkspace, clearWorkspace, hasPermission, canEdit, isAdmin],
  );
...
export function useWorkspace() {
  ...
  if (!context) {
    throw new Error("useWorkspace must be used within a WorkspaceProvider. " + "Wrap your app in <WorkspaceProvider>.");
  }
