...
// Description: Organisation management panel listing members with invite and role-assignment actions.
// Version: 1.0.0
// Modified:
...
import { Box, Card, CardContent, Typography, Button, Tabs, Tab, CircularProgress, List, ListItem, ListItemText, ListItemSecondaryAction, ListItemAvatar, Avatar, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, DialogContentText, TextField, Select, MenuItem, FormControl, InputLabel, Alert, Chip, Divider, Menu, Tooltip, Paper, useTheme, alpha } from "@mui/material";
import { Settings as SettingsIcon, Group as MembersIcon, Dashboard as OverviewIcon, Workspaces as WorkspacesIcon, Delete as DeleteIcon, Add as AddIcon, Edit as EditIcon, MoreVert as MoreIcon, Person as PersonIcon, AdminPanelSettings as AdminIcon, Star as OwnerIcon, SwapHoriz as TransferIcon, ContentCopy as CopyIcon, Check as CheckIcon } from "@mui/icons-material";
import OrganizationWorkspaces from "./OrganizationWorkspaces";

export default function OrganizationManager({ currentOrg, user, organizationModel }) {
  ...
  const userRole = typeof currentOrg?.role === "object" ? currentOrg?.role?.name?.toLowerCase() : currentOrg?.role?.toLowerCase() || "member";
  ...
  const getRoleIcon = (roleName) => {
    ...
    switch (role?.toLowerCase()) {
      case "owner":
        return <OwnerIcon fontSize="small" color="warning" />;
      case "admin":
    ...
    <Box sx={{ width: "100%" }}>
      ...
      <Card>
        ...
        <CardContent>
                ...
                <Card variant="outlined" sx={{ mb: 2 }}>
                  <CardContent>
                    ...
                    <List dense>
                      ...
                      <ListItem>
                        <ListItemText primary="Your Role" secondary={getRoleLabel(currentOrg.role)} />
                      </ListItem>
                      <ListItem>
          ...
          {/* Workspaces Tab */}
          {activeTab === 1 && currentOrg && <OrganizationWorkspaces orgId={currentOrg.id || currentOrg.org_id} orgName={currentOrg.name} userRole={userRole} />}
          ...
          {activeTab === 2 && currentOrg && (
            <Box>
              <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
                ...
                {isAdmin && (
                  <Button startIcon={<AddIcon />} variant="contained" onClick={() => setInviteModalOpen(true)}>
                    Invite Member
                  </Button>
                )}
              </Box>
                ...
                <List>
                  {members.map((member) => (
                    <ListItem key={member.id} divider>
                      <ListItemAvatar>
                        <Avatar sx={{ bgcolor: theme.palette.primary.main }}>{(member.name || member.email || "?")[0].toUpperCase()}</Avatar>
                      </ListItemAvatar>
                      <ListItemText
                        primary={
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                            <Typography fontWeight={500}>{member.name || member.email}</Typography>
                            {member.id === user?.id && <Chip label="You" size="small" color="primary" variant="outlined" />}
                          </Box>
                        }
                      ...
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1, mr: 2 }}>
                        ...
                        <Chip size="small" label={getRoleLabel(member.role)} color={getRoleLabel(member.role).toLowerCase() === "owner" ? "warning" : getRoleLabel(member.role).toLowerCase() === "admin" ? "primary" : "default"} variant="outlined" />
                      </Box>
                      {canEditMember(member) && member.id !== user?.id && (
                        <ListItemSecondaryAction>
                          <Tooltip title="Member actions">
                            <IconButton edge="end" onClick={(e) => openMemberMenu(e, member)}>
                              <MoreIcon />
                            </IconButton>
                          </Tooltip>
                        </ListItemSecondaryAction>
          ...
          {activeTab === 3 && currentOrg && (
            <Box>
              ...
              <TextField fullWidth label="Organization Name" defaultValue={currentOrg.name} sx={{ mb: 2 }} disabled={!isAdmin} />
              <TextField fullWidth label="Description" multiline rows={3} placeholder="Organization description" sx={{ mb: 2 }} disabled={!isAdmin} />
      ...
      {/* Member Actions Menu */}
      <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={closeMemberMenu} PaperProps={{ sx: { minWidth: 180 } }}>
        <MenuItem
          onClick={() => {
            closeMemberMenu();
      ...
      <Dialog open={editRoleModalOpen} onClose={() => setEditRoleModalOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth sx={{ mt: 1 }}>
            ...
            <Select value="" label="Role" onChange={(e) => handleUpdateRole(e.target.value)}>
              {roles
                .filter((r) => {
      ...
      <Dialog open={transferDialogOpen} onClose={() => setTransferDialogOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to transfer ownership to <strong>{selectedMember?.name || selectedMember?.email}</strong>? You will lose owner privileges and become an admin.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
...
function InviteModal({ open, onClose, onInvited, orgId, orgName, roles }) {
  ...
  const invitableRoles = roles.filter((role) => !["superadmin", "owner"].includes(role.name?.toLowerCase()));
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <form onSubmit={handleSubmit}>
        <DialogTitle>Invite New Member to {orgName}</DialogTitle>
        <DialogContent>
          {error && (
          ...
          {success ? (
            <Box>
              ...
              {success.invitation?.invitation_link && (
                ...
                >
                  ...
                  <IconButton size="small" onClick={() => copyToClipboard(success.invitation.invitation_link)} color={copiedLink ? "success" : "default"}>
            ...
            <>
              <TextField autoFocus margin="dense" label="Email Address" type="email" fullWidth required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="colleague@example.com" disabled={loading} />
              <FormControl fullWidth margin="dense" sx={{ mt: 2 }}>
                <InputLabel>Role</InputLabel>
                <Select value={selectedRole} label="Role" onChange={(e) => setSelectedRole(e.target.value)} disabled={loading}>
        ...
        <DialogActions>
          {success ? (
            ...
            <>
              ...
              <Button type="submit" variant="contained" disabled={loading || !email.trim() || !selectedRole}>
